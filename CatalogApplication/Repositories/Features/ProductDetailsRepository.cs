...
internal sealed class ProductDetailsRepository : BaseRepository<ProductDetailsRepository>
{
    ...
    async Task<Reply<ProductDetailsDto>> FetchDetails( Guid productId )
    {
        const string sql =
            """
                SELECT
                p.Id, p.BrandId, p.Name, p.BrandName, p.Image, p.IsFeatured, p.IsInStock, p.Price, p.SalePrice, p.ShippingPrice, p.SaleEndDate, p.ReleaseDate, p.Rating, p.NumberRatings, p.Weight, p.Dimensions,
                c.CategoryId,
                pd.Description,
                px.Xml
